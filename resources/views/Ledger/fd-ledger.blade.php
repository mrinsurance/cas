        ...
        <div class="col-md-12">
            ...
            <div class="portlet box primary">
                ...
                <div class="portlet-body">
                    <form class="form-horizontal" action="{{ route('ledger.fd-ledger') }}" method="get">
                    <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="prnt" id="record">
                            ...
                            <table class="table table-bordered table-hover" id="mytable_css">
                                ...
                                @foreach($_fd_ledgers as $ledger)
                                    ...
                                    <tr>
                                        <td>{{ \Carbon\Carbon::parse($ledger->transaction_date)->format('d-m-Y') }}</td> <!-- transaction_date -->
                                        <td>{{ $ledger->fd_no }}</td> <!-- fd_no -->
                                        <td class="align_right">{{ number_format($deposit_amount, 2) }}</td> <!-- Deposit Amount -->
                                        <td class="align_right"></td> <!-- Paid Amount -->
                                        <td class="align_right"></td> <!-- Paid Interest -->
                                        <td class="align_right">{{ number_format($running_balance, 2) }}</td> <!-- Balance -->
                                        <td class="align_right">{{ $ledger->int_rate }}</td> <!-- Interest Rate -->
                                        <td>{{ \Carbon\Carbon::parse($ledger->int_run_from)->format('d-m-Y') }}</td> <!-- Interest Run From -->
                                        <td>{{ $ledger->period_fd }}</td> <!-- Period -->
                                        <td>{{ \Carbon\Carbon::parse($ledger->maturity_date)->format('d-m-Y') }}</td> <!-- Maturity Date -->
                                        <td>{{ \Carbon\Carbon::parse($ledger->matured_on_date)->format('d-m-Y') }}</td> <!-- Matured On Date -->
                                        <td>{{ $ledger->interest_type }}</td> <!-- Type of Interest -->
                                        <td class="align_right">{{ number_format($maturity_amount, 2) }}</td> <!-- Maturity Amount -->
                                        <td>{{ $ledger->status == 1 ? 'Active' : 'Closed' }}</td> <!-- Status -->
                                    </tr>
                                    ...
                                    @if(
                                        ...
                                        @endphp
                                        <tr>
                                            <td>{{ \Carbon\Carbon::parse($ledger->matured_on_date)->format('d-m-Y') }}</td> <!-- Payout date -->
                                            <td>{{ $ledger->fd_no }}</td> <!-- FD No. -->
                                            <td class="align_right"></td> <!-- Deposit Amount -->
                                            <td class="align_right">{{ number_format($paid_amount, 2) }}</td> <!-- Paid Amount -->
                                            <td class="align_right">{{ number_format($paid_interest, 2) }}</td> <!-- Paid Interest -->
                                            <td class="align_right">{{ number_format($running_balance, 2) }}</td> <!-- Balance after payout -->
                                            <td></td> <!-- Interest Rate -->
