...
<aside class="right-side strech">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li>
          <a href="{{TRADING_URL_PURCHASE}}"> List </a>
        </li>
                ...
                <div class="panel panel-primary">
                  <div class="panel-heading">
                    ...
                    <div class="panel-body">
                        <form method="post" action="@if($item->apr_status == FALSE)
  {{TRADING_URL_PURCHASE.'/'.$item->id}} @else # @endif" id="post_frm" class="form-horizontal form-label-left"  enctype="multipart/form-data">
                          <input type="hidden" value="{{TRADING_URL_PURCHASE}}/create" id="getUrl">
                        ...
                        <div class="form-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-3">
                                      ...
                                      <input type="text" name="bill_no" class="form-control" placeholder="Bill no" required="" value="{{$item->bill_no}}">
                                    ...
                                    <div class="col-md-3">
                                      ...
                                      <select name="purchase_by" class="form-control" required="">
                                        ...
                                        <option value="CASH" {{$item->purchase_by == 'CASH' ? 'selected' : ''}}>CASH</option>
                                        <option value="SUNDRY CREDITOR" {{$item->purchase_by == 'SUNDRY CREDITOR' ? 'selected' : ''}}>SUNDRY CREDITOR</option>
                                      </select>
                                    ...
                                    <div class="col-md-3">
                                      ...
                                      <select name="purchase_party" class="form-control" required="">
                                        <option value="">Select</option>
                                        @foreach($purchasePartyList as $list)
                                        <option value="{{$list->id}}" {{$item->purchase_party_tbl_id == $list->id ? 'selected' : ''}}>{{@$list->name}}</option>
                                        @endforeach
                                      </select>
...
@endif                        
                                    ...
                                    <div class="col-md-2">
                                      <span class="text-dark">Product Name</span>
                                        <select name="product_name" class="form-control" id="product_name" required="">
                                          <option value="">--- Select ---</option>
                                            @foreach(getProductNameByTypeId($item->purchase_party_tbl_id) as $list)
                      ...
                      <div class="col-md-12">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover"  style="width: 100% !important;">
                                ...
                                <tbody>
                                  @foreach($purchaseDetail as $list)
                                  <tr id="product{{$list->id}}">
                                    ...
                                    <td class="align_right"><i class="fa fa-inr"></i> {{number_format($list->rate,2)}}</td>
                                      <td class="align_right"><i class="fa fa-inr"></i> {{$list->taxable_value }}</td>
                                    ...
                                    <td>
                                      @if($item->apr_status == FALSE)
                                      <a onclick="return confirm('Are You Sure to delete this?');" href="{{TRADING_URL_PURCHASE.'/productItem/'.$list->id}}" title="Delete"><i class="fa fa-trash text-danger"></i></a>
                                      @else
                                  ...
                                  <tr>
                                      ...
                                      <td class="align_right"><i class="fa fa-inr"></i> <strong>{{$purchaseDetail->sum('taxable_value')}}</strong></td>
                                      <td class="align_right"></td>
                                      <td class="align_right"><i class="fa fa-inr"></i> <strong>{{$purchaseDetail->sum('sgst')}}</strong></td>
                                      <td class="align_right"><i class="fa fa-inr"></i> <strong>{{$purchaseDetail->sum('cgst')}}</strong></td>
                                      <td class="align_right"><i class="fa fa-inr"></i> <strong>{{$purchaseDetail->sum('igst')}}</strong></td>
                                      <td class="align_right"><i class="fa fa-inr"></i> <strong>{{$purchaseDetail->sum('amount')}}</strong></td>
                                      <td>&nbsp;</td>
                                  </tr>
                                </tbody>
                            </table>
                  ...
                  <div class="row mb-10">
                      <div class="col-md-12 text-center">
                        <form method="post" action="{{TRADING_URL_PURCHASE}}/aproved" class="form-horizontal form-label-left" >
                        {{csrf_field()}}
                        <input type="hidden" name="product" value="{{@$item->product_type_master_tbl->name}}">
                        <input type="hidden" name="party" value="{{@$item->purchase_party_tbl->name}}">
                        <input type="hidden" name="date" value="{{$item->date_of_transaction}}">
                        <input type="hidden" name="branch" value="{{$item->branch_model_id}}">
                        <input type="hidden" name="taxValue" value="{{$purchaseDetail->sum('taxable_value')}}">
                        <input type="hidden" name="cgst" value="{{$purchaseDetail->sum('cgst')}}">
                        <input type="hidden" name="sgst" value="{{$purchaseDetail->sum('sgst')}}">
                        <input type="hidden" name="amount" value="{{$purchaseDetail->sum('amount')}}">
                        <input type="hidden" name="purchase_by" value="{{$item->purchase_by}}">
