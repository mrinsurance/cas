...
<aside class="right-side strech">
    ...
    <section class="content-header">
        <!--section starts-->
        <h1>DDS A/c List - {{$profile->member_type_model->name}} A/C No. -  {{$profile->account_no}} ({{$profile->full_name}})</h1>
        <ol class="breadcrumb">
            <li>
                <a href="{{HOME_LINK}}">
                                  ...
                                  @php
                                    ...
                                    @foreach($items as $item)
                                      <tr id="product{{$item->id}}">
                                        ...
                                        @can('product-edit')
                                            @if(!$CheckLock)
                                          <a href="{{TRANSACTION_URL_DDS_AC.''.$item->id}}/edit" class="btn btn-warning btn-sm"><i class="fa fa-edit"></i> Edit</a>
                                                @endif
                                        @endcan  
                                        </td>
                                        <td>
