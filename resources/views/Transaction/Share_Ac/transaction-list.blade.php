...
<aside class="right-side strech">
    ...
    <section class="content-header">
        <!--section starts-->
        <h1>Share A/c List - {{$profile->member_type_model->name}} A/C No. -  {{$profile->account_no}} ({{$profile->full_name}})</h1>
        <ol class="breadcrumb">
            <li>
                <a href="{{HOME_LINK}}">
            ...
            <li>
                <a href="{{TRANSACTION_URL_SHARE_AC}}">
                     Share A/C
                </a>
            </li>
                                    ...
                                    @foreach($items as $item)
                                      <tr id="product{{$item->id}}">
                                        ...
                                        <td>
                                            @if(!$CheckLock)
                                          @can('product-edit')
                                          <a href="{{TRANSACTION_URL_SHARE_AC.''.$item->id}}/edit" data-toggle="tooltip" title="Edit"><i class="fa fa-edit text-danger"></i></a>
                                          @endcan
                                          <a href="{{TRANSACTION_URL_SHARE_AC.''.$item->id}}/print" target="_blank" data-toggle="tooltip" title="Print"><i class="fa fa-fw fa-print text-success"></i></a>
                                            @endif
                                        </td>
                                        <td>
                                          {{$item->remarks}}
