...
@push('extra_js')
  ...
  <script src="{{ASSETS_JS}}rd-installment.js"></script>
  <script src="{{ASSETS_JS}}bootbox.min.js"></script>
  <script src="{{asset('assets/js/deleterd.js')}}"></script>
@endpush
...
<aside class="right-side strech">
    ...
    <section class="content-header">
        <!--section starts-->
        <h1>RD No. - {{$item->rd_no}} / Member A/C No. - {{$item->account_no}} /  ({{$ac_holder->full_name}})</h1>
        <ol class="breadcrumb">
            <li>
                <a href="{{HOME_LINK}}">
            ...
            <div class="col-md-6">
                ...
                <div class="panel panel-primary" id="hidepanel2">
                    ...
                    <div class="panel-body">
                        <form method="post" action="{{TRANSACTION_URL_POST_RD_INSTALLMENT}}" id="post_frm" class="form-horizontal form-label-left">
                    {{csrf_field()}}
                    <!-- <input type="hidden" name="ajax_url" value="{{url('transaction/recurring-deposite/blur')}}" id="ajax_url"> -->
                    <input type="hidden" name="rd_id" value="{{$item->id}}">
                    ...
                    <input type="hidden" name="rd_no" value="{{$item->rd_no}}">
                    <input type="hidden" name="back_url" value="{{Request::fullUrl()}}">
                    <input type="hidden" name="rd_monthly_amount" id="rd_monthly_amount" value="{{$item->amount}}">
                    <input type="hidden" name="paid_install" value="{{$total_paid_install}}">
                    <input type="hidden" name="period_rd" value="{{$item->period_rd}}">
                            <fieldset>
                                <!-- Email input-->
                                <div class="form-group">
                                    <label class="col-md-4 control-label" for="password">Installment Date</label>
                                    <div class="col-md-8">
                                     <input type="date" name="installment_date" class="form-control col-md-7 col-xs-12" id="date_installment" value="{{date('Y-m-d')}}">
                                   ...
                                   @endphp
                                    @foreach($installs as $val)
                                      <tr id="product{{$val->id}}">
                                        ...
                                        <td>
                                            @if(!$CheckLock)
                                          <a href="{{TRANSACTION_URL_RD_AC.''.$val->id}}/print" target="_blank" class="btn btn-success btn_sizes"><i class="fa fa-fw fa-print"></i></a>
{{--                                          @if($i == $ct)--}}
                                          <button type="button" title="Delete"  class="btn btn-danger delete-product" value="{{TRANSACTION_URL_RD_AC.''.$val->id}}">
