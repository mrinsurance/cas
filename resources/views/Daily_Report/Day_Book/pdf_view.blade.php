...
<table class="table table-bordered report-container">
    <thead>
    <tr>
        <td colspan="2" class="no-border" align="center"><h2 class="text-blue"><strong>{{$data['company_profile']->name}}</strong></h2>
            <h4>{{$data['company_profile']->address}}</h4>
            <h4>Day Book From {{date('d-M-Y',strtotime($data['from_date']))}} To {{date('d-M-Y',strtotime($data['to_date']))}}</h4>
        </td>
    </tr>
    <tr class="bg-grey">
        <th>
    ...
    <tr>
        <td>
            <table class="table table-bordered report-container">
                ...
                <tbody>
                <tr>
                    <td>{{date('d-m',strtotime($data['from_date']))}}</td>
                    <td colspan="3">Opening Cash In Hand</td>
                    <td class="text-danger must-right"><i class="fa fa-inr"></i> {{number_format($data['opening_cash_in_hand'],2)}}</td>
                </tr>
                @php
                    $bal = 0;
                    $receipt_total = 0;
                ...
                @foreach($data['gtype_groups'] as $group)
                    <tr>
                    ...
                    @foreach($data['stypes']->whereIn('gtype',[$group->gtype,strtoupper($group->gtype)])->where('type_of_transaction','Cr') as $stype)
                        @php
                            $i++;
                            $total += $stype->amount;
        ...
        <td>
            <table class="table table-bordered report-container">
                ...
                @php $bal = 0; @endphp
                <!-- Group type loop                                     -->
                @foreach($data['payment_gtype_groups'] as $group)
                    <tr>
                        <td colspan="6"><strong>{{$group->gtype}}</strong></td>
                    </tr>
                    @php $total = 0; $i = 0; $x = 0; @endphp
                    <!-- Subgroup type loop                                     -->
                    @foreach($data['payment_stypes']->whereIn('gtype',[$group->gtype,strtoupper($group->gtype)])->where('type_of_transaction','Dr') as $stype)
                        @php
                            $i++;
                            $total += $stype->amount;
    ...
    <tr class="bg-grey">
        <td class="text-right"><strong>Total = {{number_format(($receipt_total - $data['opening_cash_in_hand']),2)}}</strong></td>
