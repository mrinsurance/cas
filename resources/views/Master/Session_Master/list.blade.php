                ...
                <div class="portlet box primary">
                    <div class="panel-heading portlet-title">
                        <h3 class="panel-title pull-left add_remove_title">
                            <i class="livicon" data-name="bell" data-size="16" data-loop="true" data-c="#fff" data-hc="white"></i> Master / Session
                        </h3>
                        @can('product-create')
                        <div class="pull-right">
                            <a class="btn btn-default btn-sm" href="{{MASTER_URL_SESSION}}create"><i class="fa fa-plus-circle"></i> Add New Lead</a>
                        </div>
                        @endcan
                    </div>
                                ...
                                <tbody>
                                   @foreach($items as $item)
                                    <tr id="product{{$item->id}}">
                                      <td>{{$loop->index + 1}}</td>
                                      <td>{{date('Y',strtotime($item->start_date))}} - {{date('Y',strtotime($item->end_date))}}</td>
                                      <td>
                                        @if($item->status == 1)
                                      ...
                                      <!-- <td>
                                        <a href="{{MASTER_URL_SESSION.''.$item->id}}/edit" class="btn btn-warning btn-sm"><i class="fa fa-edit"></i> Edit</a>  <button type="button" class="btn btn-danger btn-sm delete-product" value="{{$item->id}}"><i class="fa fa-trash"></i> Delete</button>
                                      </td> -->
                                    </tr>
                                  @endforeach
