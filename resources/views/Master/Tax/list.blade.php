                ...
                <div class="portlet box primary">
                    <div class="panel-heading portlet-title">
                        <h3 class="panel-title pull-left add_remove_title">
                            <i class="livicon" data-name="bell" data-size="16" data-loop="true" data-c="#fff" data-hc="white"></i> Master / Tax
                        </h3>
                        @can('product-create')
                        <div class="pull-right">
                            <a class="btn btn-default btn-sm" href="{{MASTER_URL_TAX}}/create"><i class="fa fa-plus-circle"></i> Add New Lead</a>
                        </div>
                        @endcan
                    </div>
                                ...
                                <tbody>
                                   @foreach($items as $item)
                                      <tr id="product{{$item->id}}">
                                        ...
                                        <td>{{$item->name}}</td>
                                        <td>
                                            <a href="{{MASTER_URL_TAX.'/'.$item->id.'/edit'}}" title="Edit"  class="btn btn-success">
                                                <i class="fa fa-pencil"></i>
                                            </a>
                                            <!-- <button type="button" title="Delete"  class="btn btn-danger  delete-product" value="{{$item->id}}">
