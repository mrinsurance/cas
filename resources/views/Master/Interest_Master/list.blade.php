                ...
                <div class="portlet box primary">
                    <div class="panel-heading portlet-title">
                        ...
                        <div class="pull-right">
                            <a class="btn btn-default btn-sm" href="{{MASTER_URL_INTEREST}}create"><i class="fa fa-plus-circle"></i> Add New Interest</a>
                        </div>
                        @endcan
                    </div>
                                ...
                                <tbody>
                                   @foreach($items as $item)
                                      <tr id="product{{$item->id}}">
                                        <td>{{$loop->index + 1}}</td>
                                        <td>
                                          {{date('Y',strtotime($item->session_master_model->start_date))}} - 
                                          {{date('Y',strtotime($item->session_master_model->end_date))}}
                                        </td>
                                        <td>{{$item->saving_int}}</td>
                                        <td>{{$item->fd_int}}</td>
                                        <td>{{$item->rd_int}}</td>
                                        <td>{{$item->mis_int}}</td>
                                        <td>{{$item->loan_int}}</td>
                                        <td>{{$item->loan_default_int}}</td>
                                        <td>
                                            @can('product-edit')
                                          <a href="{{MASTER_URL_INTEREST.''.$item->id}}/edit" class="btn btn-info btn-sm"><i class="fa fa-edit"></i> Edit</a>
                                          @endcan
                                        </td>
                                      </tr>
                                    @endforeach
