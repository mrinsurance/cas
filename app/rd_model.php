...
class rd_model extends Model
{
    protected $table = 'rd_models';
    ...
    protected static function boot()
    {
        ...
        static::addGlobalScope('agentFilter', function (Builder $builder) {
            if (Auth::check() && Auth::user()->staff_type == 'Agent') {
                ...
                $builder->join('user_open_new_ac_model', 'rd_models.open_new_ac_model_id', '=', 'user_open_new_ac_model.open_new_ac_model_id')
                    ->where('user_open_new_ac_model.user_id', Auth::id())
                    ->select('rd_models.*'); // Ensure proper selection
            }
        });
    }
    ...
    public function rd_installment_model()
    {
    	return $this->hasMany('App\rd_installment_model');
    }
}
