...
class saving_ac_model extends Model
{
    protected $table = 'saving_ac_models';
    ...
    protected static function boot()
    {
        ...
        static::addGlobalScope('agentFilter', function (Builder $builder) {
            if (Auth::check() && Auth::user()->staff_type == 'Agent') {
                ...
                $builder->join('user_open_new_ac_model', 'saving_ac_models.open_new_ac_model_id', '=', 'user_open_new_ac_model.open_new_ac_model_id')
                    ->where('user_open_new_ac_model.user_id', Auth::id())
                    ->select('saving_ac_models.*'); // Ensure proper selection
            }
        });
    }
