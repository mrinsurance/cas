...
class dds_ac_model extends Model
{
    protected $table = 'dds_ac_models';
    ...
    protected static function boot()
    {
        ...
        static::addGlobalScope('agentFilter', function (Builder $builder) {
            if (Auth::check() && Auth::user()->staff_type == 'Agent') {
                ...
                $builder->join('user_open_new_ac_model', 'dds_ac_models.open_new_ac_model_id', '=', 'user_open_new_ac_model.open_new_ac_model_id')
                    ->where('user_open_new_ac_model.user_id', Auth::id())
                    ->select('dds_ac_models.*'); // Ensure proper selection
            }
        });
    }
